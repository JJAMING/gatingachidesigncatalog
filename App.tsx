...
function getQtyOptions(id: string, size?: string) {
    if (id === '명함') return [500, 700, 1000, 2000, 3000, 5000, 10000];
    ...
    if (id === '보증서') return [1000, 2000, 3000, 5000];
    return [10, 20, 30, 50, 100, 200, 500];
}
function getMinQty(id: string, size?: string) {
    if (id === '명함') return 500;
    if (id === '명찰') return 1;
...
const DetailModal: React.FC<{
    ...
    const [qty, setQty] = useState(getMinQty(item.id, getSizes(item.id)[0]));
                    ...
                    <div className="space-y-1.5">
                        ...
                        <div className="relative">
                            <select value={size} onChange={e => {
                                ...
                                setSize(newSize);
                                const newMin = getMinQty(item.id, newSize);
                                if (qty < newMin) setQty(newMin);
                                const newOpts = getQtyOptions(item.id, newSize);
                                if (!newOpts.includes(qty) && qty >= newMin) {
                                    const closest = newOpts.reduce((prev, curr) => Math.abs(curr - qty) < Math.abs(prev - qty) ? curr : prev);
                                    setQty(closest);
                                }
                            }}
